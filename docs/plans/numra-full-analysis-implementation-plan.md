# ⚠️ SUPERSEDED — Siehe `docs/plans/numra-full-analysis-execution-plan.md`

> Dieses Dokument wurde durch den **Execution Plan** ersetzt:
> **`docs/plans/numra-full-analysis-execution-plan.md`**
>
> Der Execution Plan enthält alle sieben Reconciliation-Patches:
>
> 1. ADR 0017 als Sequenz-Governance respektiert
> 2. Bereits erledigte Aufgaben als Baseline markiert
> 3. Dokument als Execution Plan (nicht zweite kanonische Quelle) klassifiziert
> 4. Präzisierte "Shared Models"-Regel
> 5. Vollständiger Report-Envelope V3 + ClaimV3
> 6. Idempotenz-Kryptovertrag
> 7. Token-Limit als projektinterne Grenze dokumentiert
>
> **Dieses Dokument bitte nicht mehr als Quelle verwenden.**
...
**Leitprinzip:** Keine geteilten Modelle, Unions oder Endpunkte zwischen V1 und V2/V3. Jede geteilte Struktur ist ein potenzieller stiller Breaking Change.
...
**Einziger V1-Guard-Fix:** `policy.version != "v1"` → 422 `METHOD_VERSION_MISMATCH`
...
src/numerology_api/routes/profiles.py
...
src/numerology_agent/service_v3.py
...
Keine unbekannten oder doppelten Section-IDs.
...
Enthält ausschließlich den kanonischen Berichtinhalt:

```
schema_version, summary, sections, limitations, suggestions
```
...
```
Welle 0  →  ADRs + Plan-Grundlage
Welle 1  →  API-Grundlage (Profile, Meta, Interfaces, OpenAPI, CLI)
...
**Ziel:** Architekturentscheidungen dokumentieren, bevor Code geschrieben wird.

**15 Schritte (parallelisierbar):**

1. ADR 0017: `pythagorean-v2` als künftiger Default (Web-Default-Wechsel erst Welle 5C)
2. ADR 0018: Strikt paralleler Stack `/api/v2/*` — keine geteilten Unions/Modelle
...
| --- | ------------------------------------ | -------------------------------------------------------------- | ------------ |
| 1   | V1-Guard-Fix (`version!="v1"` → 422) | `routes/profiles.py`                                           | —            |
| 2   | V2-Profil-Endpunkt                   | `routes/profiles_v2.py` (neu)                                  | —            |
| 3   | V2-Meta-Endpunkt (`MetaResponseV2`)  | `routes/meta_v2.py` (neu)                                      | —            |
| 4   | V2-Analyse-Runtime                   | `analysis_runtime_v2.py` (neu)                                 | —            |
| 5   | Dependency-Wiring-Interfaces         | `provider_v3.py`, `idempotency.py`, `dependencies_v3.py` (neu) | —            |
| 6   | `create_app` erweitern               | `app.py`                                                       | Schritt 5    |
| 7   | OpenAPI umstellen                    | `export_openapi.py`, `package.json`                            | Schritt 2+3  |
| 8   | CLI `--method-version`               | `main.py`                                                      | —            |

**Verifikation:**

- `uv run pytest` — bestehende Suite grün
- `/api/v2/profiles/calculate` → 200 (Lukas)
- `/api/v1/profiles/calculate` mit `version="v2"` → 422
...
| #   | Schritt                   | Datei                                           | Abhängigkeit |
| --- | ------------------------- | ----------------------------------------------- | ------------ |
| 1   | V3-Fact-Modelle           | `models_v3.py` (neu, Teil 1)                    | —            |
| 2   | Fact-Package-Builder      | `service_v3.py` (neu, Teil 1)                   | Schritt 1    |
| 3   | V3-Wissensmodell          | `numerology_knowledge/models_v3.py` (neu)       | —            |
| 4   | V3-Wissensdaten           | `data/de-v3.json` (neu)                         | Schritt 3    |
| 5   | V3-Wissensloader          | `loader_v3.py` (neu)                            | Schritt 3+4  |
| 6   | V3-Interpretation         | `numerology_interpretation/service_v3.py` (neu) | Schritt 5    |
...
| #   | Schritt                     | Datei                                                                   | Abhängigkeit  |
| --- | --------------------------- | ----------------------------------------------------------------------- | ------------- |
| 1   | V3-Report-Modelle           | `models_v3.py` (erweitern)                                              | Welle 2       |
| 2   | V3-Prompts + Loader         | `prompts_v3.py`, `de-report-system-v3.md`, `de-report-task-v3.md` (neu) | —             |
| 3   | V3-Provider (finish_reason) | `deepseek_v3.py` (neu)                                                  | Schritt 1+2   |
| 4   | V3-Agent-Service            | `service_v3.py` (erweitern, Teil 2)                                     | Schritt 3     |
| 5   | Idempotenz-Implementierung  | `idempotency.py` (erweitern)                                            | Welle 1       |
| 6   | V2-Analyse-Routen           | `routes/analyses_v2.py` (neu)                                           | Schritt 4+5   |
| 7   | Größenbudget-Tests          | (Tests)                                                                 | Schritt 6     |
| 8   | Wheel-Smoke-Test            | `tests/deployment/test_wheel_resources.py` (neu)                        | Schritt 1+2+4 |
...
- Wheel-Smoke: V3-Ressourcen im Wheel vorhanden und lesbar

---

### Welle 4 — Web-Migration
...
| #   | Schritt                     | Datei                                                         | Abhängigkeit |
| --- | --------------------------- | ------------------------------------------------------------- | ------------ |
| 1   | Präsentationsmodell-Adapter | `features/profile/presentation.ts` (neu)                      | —            |
| 2   | API-Client V2-Endpunkte     | `api/client.ts`, `api/schema.d.ts`                            | Welle 1      |
| 3   | Section-Mapping (18→9)      | `features/report/sectionMapping.ts` (neu)                     | —            |
| 4   | ResultsTabs (WAI-ARIA)      | `features/report/ResultsTabs.tsx` (neu)                       | Schritt 3    |
| 5   | ReportExperience umbauen    | `features/report/ReportExperience.tsx`                        | Schritt 4    |
| 6   | Druckansicht + PDF          | `features/report/PrintView.tsx` (neu), `export/profilePdf.ts` | Schritt 1    |
| 7   | Storage Schema v4           | `storage/database.ts`, `storage/repository.ts`                | —            |
| 8   | Offline-Status              | `pwa/offlineState.ts` (neu)                                   | —            |
| 9   | AnalysisWizard v1/v2        | `features/analysis/AnalysisWizard.tsx`                        | Schritt 2    |
...
- `max_output_tokens` kalibrieren (≤32.768)
...
Nur bei vollständig bestandenen Gates:
...
| Welle | Datei                                                                                                                                                                                                               |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0     | `docs/adr/0017–0027` (11), `docs/methods/capability-matrix-v3.md`, `docs/methods/section-budgets-v3.md`                                                                                                             |
| 1     | `routes/profiles_v2.py`, `routes/meta_v2.py`, `analysis_runtime_v2.py`, `idempotency.py`, `dependencies_v3.py`, `agent/provider_v3.py`                                                                              |
| 2     | `agent/models_v3.py`, `agent/service_v3.py` (T1), `knowledge/models_v3.py`, `knowledge/loader_v3.py`, `knowledge/data/de-v3.json`, `interpretation/service_v3.py`                                                   |
| 3     | `agent/deepseek_v3.py`, `agent/prompts_v3.py`, `prompt_templates/system/de-report-system-v3.md`, `prompt_templates/tasks/de-report-task-v3.md`, `routes/analyses_v2.py`, `tests/deployment/test_wheel_resources.py` |
| 4     | `web/features/profile/presentation.ts`, `web/features/report/sectionMapping.ts`, `web/features/report/ResultsTabs.tsx`, `web/features/report/PrintView.tsx`, `web/pwa/offlineState.ts`                              |
...
| ----- | ------------------------------------------ | -------------------------- |
| 1     | `routes/profiles.py`                       | V1-Guard-Fix               |
| 1     | `app.py`                                   | `create_app()`-Signatur    |
| 1     | `scripts/export_openapi.py`                | Ausgabeziele               |
| 1     | `apps/web/package.json`                    | generate-api-Pfad          |
| 1     | `cli/main.py`                              | --method-version           |
| 3     | `agent/models_v3.py`                       | Report-Modelle             |
| 3     | `agent/service_v3.py`                      | AgentServiceV3 (T2)        |
| 3     | `idempotency.py`                           | RedisIdempotencyStoreV3    |
| 4     | `web/api/client.ts`                        | V2-Endpunkte               |
| 4     | `web/api/schema.d.ts`                      | Regenerierung              |
| 4     | `web/features/report/ReportExperience.tsx` | Tabs integrieren           |
| 4     | `web/features/export/profilePdf.ts`        | Präsentationsmodell        |
| 4     | `web/storage/database.ts`                  | Schema v4                  |
| 4     | `web/storage/repository.ts`                | Mehrfachberichte, reportId |
| 4     | `web/features/analysis/AnalysisWizard.tsx` | v1/v2-Konfiguration        |

---

## Rollback-Strategie

- `/api/v2/analyses/*`-Router aus `app.py` deaktivieren → V1 unberührt
