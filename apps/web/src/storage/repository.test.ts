...
function repository(vault = new MemoryVault()) {
    ...
    return {
        database,
        repository: new LocalProfileRepository(database, vault),
        vault,
    };
}
...
describe("LocalProfileRepository", () => {
    ...
    it("requires explicit opt-in and supports search, sorting and deletion", async () => {
        ...
        await expect(
            profiles.saveProfile(profile("Max Mustermann", "a"), false),
        ).rejects.toThrow(/Opt-in/);

        const max = await profiles.saveProfile(
            profile("Max Mustermann", "a"),
            true,
        );
        await profiles.saveProfile(profile("Anna Beispiel", "b"), true);

        expect(
            (await profiles.listProfiles({ query: "max", sort: "name" }))[0]
                ?.name,
        ).toBe("Max Mustermann");
        expect(
            (await profiles.listProfiles({ query: "", sort: "name" }))[0]?.name,
        ).toBe("Anna Beispiel");
    ...
    it("stores protected payloads without plaintext and requires unlock", async () => {
        ...
        vault.lock();
        await expect(profiles.getProfile(saved.id)).rejects.toBeInstanceOf(
            VaultLockedError,
        );
    ...
    it("never writes plaintext when a persisted vault is locked after restart", async () => {
        ...
        const restarted = new LocalProfileRepository(
            state.database,
            new MemoryVault(),
        );
        await expect(
    ...
    it("exports, imports and completely deletes local data", async () => {
        ...
        expect(
            await target.repository.listProfiles({
                query: "",
                sort: "updated",
            }),
        ).toHaveLength(1);
    ...
    it("round-trips every payload from one protected vault into another", async () => {
        ...
        const saved = await source.repository.saveProfile(
            profile("Protected Export", "z"),
            true,
        );
        const report = {
        ...
        expect(await target.repository.listFollowUps(saved.id)).toEqual([
            followUp,
        ]);
        expect(await target.repository.getNote(saved.id)).toBe(
            "Protected note",
        );
        expect(
            JSON.stringify(await target.database.reports.toArray()),
        ).not.toContain("Protected report");
        expect(
            JSON.stringify(await target.database.notes.toArray()),
        ).not.toContain("Protected note");
    });
    ...
    it("migrates an unprotected V1 archive and rejects unrecoverable protected V1 payloads", async () => {
        ...
        const legacyArchive = {
            ...encrypted,
            format: "numra-export-v1" as const,
        };
        const target = repository();
    ...
    it("recovers protected V1 payloads inside their original unlocked vault", async () => {
        ...
        await source.repository.saveReport(saved.id, report, true);
        await source.repository.saveFollowUp(saved.id, followUp, true);
        await source.repository.saveNote(
            saved.id,
            "Legacy protected note",
            true,
        );
        ...
        expect(await source.repository.listFollowUps(saved.id)).toEqual([
            followUp,
        ]);
        expect(await source.repository.getNote(saved.id)).toBe(
            "Legacy protected note",
        );
    });
    ...
    it("enforces one local report and two local follow-ups per profile", async () => {
        const state = repository();
        const saved = await state.repository.saveProfile(
            profile("Quota Name", "q"),
            true,
        );
        const report = {
            schema_version: "analysis-report-v2",
        ...
        ).rejects.toThrow(/zwei Rückfragen/);
    });
    ...
    it("creates, updates and removes a local note for a profile", async () => {
        ...
        await state.repository.saveNote(
            saved.id,
            "Eine persönliche Reflexion.",
            true,
        );
        expect(await state.repository.getNote(saved.id)).toBe(
            "Eine persönliche Reflexion.",
        );
