...
const formSchema = z
  .object({
    coreName: z.string().trim().min(1, "Bitte gib deinen Geburtsnamen ein.").max(200),
  ...
  .superRefine((value, context) => {
    ...
    if (value.birthDate > value.asOfDate) {
      context.addIssue({
        code: "custom",
        path: ["birthDate"],
        message: "Das Geburtsdatum darf nicht nach dem Berechnungsdatum liegen.",
      });
    }
  });
...
function toRequest(values: FormValues): ProfileCalculationRequest {
  return {
    person: {
  ...
  return (
    <section className="analysis-layout">
      <aside className="step-rail" aria-label="Fortschritt">
        ...
        <ol>
          <li className={step === 1 ? "active" : "complete"}><span>01</span>Daten</li>
          <li className={step === 2 ? "active" : ""}><span>02</span>Prüfung</li>
          <li><span>03</span>Atlas</li>
        </ol>
        <p>Deine Eingaben werden für die Berechnung übertragen, aber nicht serverseitig gespeichert.</p>
      </aside>
      ...
      <div className="analysis-panel">
        {step === 1 ? (
          <>
            ...
            <p className="panel-lead">Der Geburtsname bildet das Kernprofil. Ein aktiver Name bleibt davon vollständig getrennt.</p>
            {!online && <div className="notice notice-warning">Offline keine neue Berechnung möglich. Gespeicherte Profile bleiben später lesbar.</div>}
            <form
              onSubmit={(event) => {
            ...
            >
              <label>
                ...
                <input autoComplete="name" {...register("coreName")} />
                {errors.coreName && <small role="alert">{errors.coreName.message}</small>}
              </label>
              <label>
                <span>Aktuell verwendeter Name <em>optional</em></span>
                <input autoComplete="off" {...register("activeName")} />
              </label>
              ...
              {errors.ageConfirmed && <small role="alert">{errors.ageConfirmed.message}</small>}
              <button className="button button-primary" type="submit" disabled={!online}>Eingaben prüfen</button>
            </form>
          </>
        ) : (
          <>
            ...
            <p className="panel-lead">Kontrolliere die Daten. Die Methodenpolicy ist fest und wird mit dem Ergebnis versioniert.</p>
            <dl className="review-grid">
              <div><dt>Geburtsname</dt><dd>{review?.coreName}</dd></div>
              <div><dt>Aktiver Name</dt><dd>{review?.activeName || "Nicht angegeben"}</dd></div>
              <div><dt>Geboren</dt><dd>{review?.birthDate}</dd></div>
              <div><dt>Stand</dt><dd>{review?.asOfDate}</dd></div>
            </dl>
            <div className="method-stamp">
              <span>PYTHAGOREAN · V1</span>
              <strong>Deterministisch · Meisterzahlen 11 / 22 / 33</strong>
            </div>
            <label className="check-row consent-row">
              <input
                type="checkbox"
                checked={methodAccepted}
                onChange={(event) => setMethodAccepted(event.target.checked)}
              />
              <span>Ich verstehe, dass Numerologie eine symbolische Reflexionsmethode und keine wissenschaftliche Diagnose ist.</span>
            </label>
            {apiError && <div className="notice notice-error" role="alert">{apiError}</div>}
            <div className="wizard-actions">
              <button className="button button-quiet" type="button" onClick={() => setStep(1)}>Zurück</button>
